...
## Additional mandatory capabilities from the expansion specification

| Capability | Category | Reference Project | Description | Agent | Tool | Risk | Permission | Level | Status | Dependencies | Test | Notes |
|---|---|---|---|---|---|---|---|---:|---|---|---|---|
| Event Bus | Core runtime | Project requirement | Typed events connecting input, tasks, tools, permissions, system and UI | Orchestrator | event_bus | SAFE | runtime.events | 2 | PLANNED | task/event contracts | event ordering and delivery tests | Must remain UI-independent |
| Global state machine | Core runtime | Project requirement | OFFLINE through READY/SLEEPING/EXECUTING/ERROR/SHUTTING_DOWN | Orchestrator | state_manager | SAFE | runtime.state | 2→4 | PLANNED | event bus | transition tests | Invalid transitions rejected |
| Multi-task background runtime | Agents | Project requirement | Multiple concurrent tasks with IDs, priority, dependencies, cancellation and progress | Orchestrator | task_runtime | REVERSIBLE | tasks.background | 2 | PLANNED | queue, scheduler, policy | end-to-end task lifecycle | Safe parallelism only |
| Minimum 15 real agents | Agents | Project requirement | Fifteen specialized agents with real responsibilities and contracts | Orchestrator | agent_registry | SENSITIVE | agent.execute | 2→6 | PLANNED | task runtime, policy | per-agent contract/integration tests | No placeholder agents |
| Application discovery | Windows | Project requirement | Discover installed apps from multiple Windows sources | Computer Control | app_discovery | SAFE | system.discovery | 3 | PLANNED | Windows APIs | fixture + live Windows tests | Never assume installed apps |
| Application Registry | Windows | Project requirement | Metadata, adapter, actions, permissions, health and state | Computer Control | app_registry | SAFE | apps.registry | 3 | PLANNED | discovery, adapters | registry contract tests | Discovery does not grant execution |
| Application adapter fallback chain | Windows | Project requirement | API/CLI → protocol → automation → GUI → vision/input → assisted | Computer Control | app_adapter | SENSITIVE | apps.control | 3→4 | PLANNED | policy, UI automation, vision | adapter fallback tests | Prefer stable APIs |
| Permission Broker | Security | Project requirement | Central configurable permission decisions for every capability | Security/Policy | permission_broker | HIGH | policy.manage | 2→6 | PLANNED | risk model, identity | deny/allow/confirm tests | LLM cannot self-approve |
| Computer Context | Awareness | Project requirement | Active app/window/project/browser/clipboard/audio/task context | System Monitor | computer_context | SENSITIVE | awareness.read | 3→5 | PLANNED | Windows adapters | snapshot consistency tests | Only authorized data |
| Action Journal | Reversibility | Project requirement | Records safe action metadata, result and exact undo operation | QA/Verification | action_journal | SENSITIVE | audit.write | 2→6 | PLANNED | task runtime, storage | undo/replay tests | Never invent prior state |
| Self-diagnostic health report | Diagnostics | Project requirement | Checks local dependencies and optional capabilities with capability-level status | QA/Verification | health_report | SAFE | diagnostics.read | 1→6 | IN_PROGRESS | diagnostics | healthy/degraded/failure fixtures | Level 1 CLI foundation exists |
| Self-awareness registry | Awareness | Project requirement | Dynamic model/tools/agents/apps/devices/permissions/capabilities state | Memory | awareness_registry | SAFE | awareness.read | 5→6 | PLANNED | registries, hardware | stale-state tests | No false static claims |
| Ollama model capability routing | LLM | Project requirement | Select fast/reasoning/code/vision models without replacing provider abstraction | Orchestrator | model_router | SAFE | model.select | 5 | PLANNED | hardware/model metadata | routing matrix tests | No automatic giant downloads |
| Hardware-adaptive model selection | LLM/hardware | Project requirement | Adapt model choice to CPU/RAM/GPU/VRAM and available resources | System Monitor | hardware_probe | SAFE | hardware.read | 3→5 | PLANNED | telemetry, model metadata | constrained fixture tests | Inspect real hardware first |
| Local clap activation | Voice | Project requirement | Two local clap detections activate wake flow | Voice | clap_detector | SAFE | voice.wake | 4 | PLANNED | audio input, VAD | prerecorded clap/noise tests | Configurable sensitivity and interval |
| Ambient UI state experience | UI | Project requirement | Non-invasive orb/core/waveform/contextual visual presence | Voice/UI | ambient_ui | SAFE | ui.display | 6 | PLANNED | event bus, state machine | state visualization tests | Original design, not a dashboard clone |
| Show-activity mode | UI | Project requirement | Explain task/agent/tool progress without secrets | Orchestrator | activity_view | SAFE | ui.activity | 6 | PLANNED | audit redaction | redaction tests | Sensitive data must be hidden |
| Graceful degradation | Runtime | Project requirement | Optional capability failure must not kill local text Core | Orchestrator | capability_manager | SAFE | runtime.health | 1→6 | IN_PROGRESS | diagnostics | fault injection tests | Text remains available |
| Self-healing with bounded recovery | Runtime | Project requirement | Diagnose and retry/replan without changing security policy | Recovery | recovery_engine | SENSITIVE | recovery.execute | 2→6 | PLANNED | QA, policy, task state | bounded retry/loop tests | Security changes never automatic |
| Local notification manager | Automation | Project requirement | Local task completion, alert and permission notifications | Automation | notification_manager | SAFE | notifications.write | 5→6 | PLANNED | scheduler, UI | notification tests | No cloud required |
| Clipboard intelligence | Windows | Project requirement | Contextual translate/summarize/explain/transform copied text | Clipboard | clipboard_tools | SENSITIVE | clipboard.read | 3→5 | PLANNED | clipboard adapter, memory | privacy/size tests | Explicit user enablement |
| Multimedia control | Windows/media | Project requirement | Local media search/play/pause/volume/next controls | Media | media_tools | REVERSIBLE | media.control | 3 | PLANNED | Windows media APIs | mocked + Windows tests | External services optional |
| Communications adapters | Integrations | Project requirement | Email/WhatsApp/Telegram/Discord via adapters | Communication | communication_tools | EXTERNAL_SIDE_EFFECT | communications.send | 6 | PLANNED | credentials, confirmation | mocked send tests | Explicit confirmation for sends |
| Smart-home adapter | Integrations | Project requirement | Home Assistant and compatible device abstraction | Automation | device_adapter | EXTERNAL_SIDE_EFFECT | devices.control | 6 | PLANNED | pairing, policy | mock device tests | No assumption that devices exist |
| Phone adapter | Integrations | Project requirement | Notifications/messages/calls/battery/etc. with explicit permissions | Remote | phone_adapter | SENSITIVE | phone.access | 6 | PLANNED | pairing/authentication | permission tests | Optional |
| Remote gateway | Remote | Project requirement | Authenticated remote status/task/control interface | Remote | remote_gateway | CRITICAL | remote.control | 6 | PLANNED | pairing, encryption, policy | auth/authorization tests | Default should remain local |
| Autostart manager | Installation | Project requirement | Install/uninstall/recovery for Windows startup | Automation | autostart_manager | SENSITIVE | startup.manage | 6 | PLANNED | installer | install/uninstall tests | Explicit user authorization |
| Installer/update lifecycle | Installation | Project requirement | Real install/start/stop/diagnose/update/uninstall lifecycle | Installer | installer | HIGH | installation.manage | 6 | PLANNED | packaging, autostart | clean-machine tests | Scripts cannot be placeholders |
| Dynamic tool/plugin discovery | Extensibility | Project requirement | Discover, validate and activate extensions safely | Plugin | plugin_manager | SENSITIVE | plugins.manage | 5→6 | PLANNED | manifest/schema/isolation | malicious-manifest tests | Discovery never grants permission |
| Provenance-aware answers | Intelligence | Project requirement | Distinguish model knowledge, local data, tool output and web data | Research | provenance | SAFE | provenance.read | 5 | PLANNED | context, research | source attribution tests | Never blur source types |
| Document artifact pipeline | Documents | Project requirement | Generate/transform/compare documents and artifacts | Document | artifact_tools | SENSITIVE | files.write | 5 | PLANNED | parsers, filesystem policy | fixture corpus | Format-specific tests |
| Code project lifecycle | Development | Project requirement | Inspect → edit → test → debug → document → optional commit | Coding/Git | dev_tools | HIGH | dev.execute | 5 | PLANNED | sandbox, Git, QA | isolated project E2E | Destructive Git actions gated |
| System graceful startup | Runtime | Project requirement | Core, health checks and optional services start independently | Orchestrator | startup_manager | SAFE | runtime.start | 6 | PLANNED | diagnostics, capability registry | service failure matrix | Optional failures degrade |
